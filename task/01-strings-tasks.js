...
function getRectangleString(width, height) {
    ...
    return topBorder + middleLine.repeat(height - 2) + bottomBorder;
}
...
function encodeToRot13(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        let currentUIndex = str.charCodeAt(i);
        let newUIndex = currentUIndex + 13;

        if (currentUIndex < 65 || (currentUIndex > 90 && currentUIndex < 97) || currentUIndex > 122) {
            result += str[i];
            continue;
        }

        if (newUIndex > 90 && newUIndex < 104) {
            newUIndex = newUIndex - 90 + 64;
        }
        else if (newUIndex > 122 && newUIndex < 136) {
            newUIndex = newUIndex - 122 + 96;
        }

        result += String.fromCharCode(newUIndex);
    }
